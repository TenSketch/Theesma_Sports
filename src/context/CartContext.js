...
export function CartProvider({ children }) {
  ...
  const addToCart = (product, quantity = 1, size = 'M') => {
    setCartItems(prev => {
      ...
      const existing = prev.find(item => (item.id === id || item._id === id) && item.size === size);
      if (existing) {
      ...
      return [...prev, { ...product, id: id, quantity, size }];
    });
  };
  ...
  const updateQuantity = (id, size, quantity) => {
    ...
    setCartItems(prev => prev.map(item => 
      ((item.id === id || item._id === id) && item.size === size) ? { ...item, quantity } : item
    ));
  };
