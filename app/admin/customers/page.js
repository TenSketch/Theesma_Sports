...
import { Users, Search, MoreHorizontal, Mail, ShieldCheck, MailPlus, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        ...
        console.error('Directory Sync Failure', e);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
         ...
         <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.3em] animate-pulse">Retrieving member records...</p>
      </div>
    );
  }
          ...
          <tbody className="divide-y divide-white/5">
            {filteredCustomers.map((c) => (
              <tr key={c.id} className="group hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-8 py-6 text-sm font-bold font-mono">₹{c.totalSpent?.toLocaleString() || '0'}</td>
                <td className="px-8 py-6 text-sm font-bold text-gray-400">{c.orderCount || 0} Manifests</td>
                <td className="px-8 py-6">
