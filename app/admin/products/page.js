    ...
    <div className="space-y-8">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white/5 border border-white/10 p-1 flex-shrink-0 cursor-pointer" onClick={() => {
                      const images = product.images?.length ? product.images : [product.image || '/img/gear.png'];
                      setLightboxImages(images);
                      setLightboxIndex(0);
                      setLightboxOpen(true);
                    }}>
                       <img src={product.images?.[0] || product.image || '/img/gear.png'} alt={product.name} className="w-full h-full object-cover transition-opacity" />
                    </div>
                    <div>
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center px-4 py-8">
          <div className="relative w-full max-w-4xl bg-black/95 border border-white/10 rounded-3xl overflow-hidden">
            <button
              onClick={() => setLightboxOpen(false)}
              className="absolute top-4 right-4 z-20 w-10 h-10 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-white hover:bg-white/20"
            >
              ✕
            </button>
            <div className="relative h-[70vh] bg-black">
              ...
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 border border-white/10 text-white flex items-center justify-center hover:bg-white/20"
              >
                ‹
              </button>
              <button
                onClick={() => setLightboxIndex((lightboxIndex + 1) % lightboxImages.length)}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 border border-white/10 text-white flex items-center justify-center hover:bg-white/20"
              >
                ›
              </button>
            </div>
            <div className="flex items-center justify-center gap-3 p-4 bg-white/5 overflow-x-auto">
              {lightboxImages.map((src, index) => (
                <button
                  key={src + index}
                  onClick={() => setLightboxIndex(index)}
                  className={`h-20 w-20 rounded-xl overflow-hidden border ${index === lightboxIndex ? 'border-brand-blue' : 'border-white/10'}`}
                >
